<?php
header('Content-Type: application/json');
include "../../../../config/routes.php";
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Includes 
    include MODELS . "/Helpers.php";
    include CONFIG . "/connection.php";
    include MODELS . "/Validation.php";
    include MODELS . "/Patterns.php";
    include "../Admin.php";

    // Variables
    $brand = $_POST['ddlBrand'];
    $cat = $_POST['ddlCat'];
    $color = $_POST['ddlColor'];
    $sizes = isset($_POST['cbSize']) ? $_POST['cbSize'] : null;
    $name = $_POST['inName'];
    $price = $_POST['inPrice'];
    $desc = $_POST['taDesc'];
    $id = $_POST['inHidden'];

    $errs = [];
    $errCodes = [];
    $success;

    if (isset($_FILES['fileImg']) && $_FILES['fileImg']['error'] == 0) {
        // Image wasn't uploaded
        $img = $_FILES['fileImg'];
    }



    // // Validation
    $valid = new Validation();
    // $valid->name("Glove name")->value($name)->customPattern($regGloveName)->required();
    // $valid->name("Price")->value($price)->customPattern($regPrice)->required();
    // $valid->name("Description")->value($desc)->required();

    if (!isset($brand) || empty($brand)) {
        // Brand must be selected
        $errs[] = "Must select a brand";
        $errCodes[] = 422;
    }
    if (!isset($cat) || empty($cat)) {
        // Category must be selected
        $errs[] = "Must select a category";
        $errCodes[] = 422;
    }
    if (!isset($color) || empty($color)) {
        // Color must be selected
        $errs[] = "Must select a color";
        $errCodes[] = 422;
    }

    // Check if dropdowns/sizes/form are all valid
    if (!empty($errs)) {
        // Errors array is not empty, not all drop downs were selected
        $errs[] = "Every dropdown must be selected";
        $errCodes[] = 422;
    }
    if (!isset($sizes) || empty($sizes)) {
        // Size must be selected
        $errs[] = "At least one size must be selected";
        $errCodes[] = 422;
    }
    if (false) {
        // Validation failed
        // Unprocessable entity
        $errs[] = $valid->errors;
        $errCodes[] = 422;
    }

    if (empty($errs)) {
        // Validation success
        // Try
        try {
            //Begin Transaction
            $conn->beginTransaction();

            if (isset($img)) {
                // Image was uploaded
                // Image parts
                // This is checked only if image is set, because image is OPTIONAL by default
                $n_name = time() . "__" . $img['name'];
                $n_type = $img['type'];
                $n_tmp = $img['tmp_name'];
                $n_size = $img['size'];

                // Allowed types
                $allowed_types = ['image/jpg', 'image/jpeg', 'image/png'];

                // Check for image errors
                if (!in_array($n_type, $allowed_types)) {
                    $errs[] = "Image type is not valid";
                    $errCodes[] = 422;
                }
                if ($n_size > 3000000) {
                    $errs[] = "Uploaded image is too large";
                    $errCodes[] = 422;
                }
            }

            if (!count($errs)) {

                if (isset($img)) {
                    // Image was uploaded, OPTIONAL
                    // Creating thumnail image
                    list($n_w, $n_h) = getimagesize($n_tmp);
                    $n_img = imagecreatefromstring(file_get_contents($n_tmp));
                    $t_w = 150;
                    $t_h = ($t_w / $n_w) * $n_h;
                    $t_img = imagecreatetruecolor($t_w, $t_h);
                    imagecopyresampled($t_img, $n_img, 0, 0, 0, 0, $t_w, $t_h, $n_w, $n_h);
                    // Definig name/path
                    $t_name = "thumb__" . $n_name;
                    $t_path = "assets/img/gloves/thumbnail/{$t_name}";

                    // Upload thumbnail img
                    switch ($n_type) {
                        case "image/png":
                            imagepng($t_img, '../../../../' . $t_path);
                            break;
                        default:
                            imagejpeg($t_img, '../../../../' . $t_path);
                    }

                    // Upload normal img
                    $n_path = "assets/img/gloves/normal/" . $n_name;

                    if (isset($img)) {
                        $isUploaded = move_uploaded_file($n_tmp, '../../../../' . $n_path);
                        if (!$isUploaded) {
                            // Failed to upload image
                            $errs[] = "Failed to upload the image";
                            $errCodes[] = 422;
                        }
                    }
                }

                // Is there a active glove with that id
                $isGlove = isGloveAndActive($id);
                if (!$isGlove) {
                    $errs[] = "There is no active glove with that ID";
                    $errCodes[] = 404;
                } else {
                    $glove = selectCurrentGloveValues($id);
                    $removeID = $glove->id;
                    $isGloveRemove = removeGlove($removeID);
                    if (!$isGloveRemove) {
                        // Failed to remove previous glove  (DATE REMOVED to NULL)
                        $errs[] = "Failed to insert glove";
                        $errCodes[] = 422;
                    } else {
                        if (!isset($img)) {
                            // Image is not set so we need to get current image values
                            $n_name = $glove->n_img;
                            $t_name = $glove->t_img;
                        }

                        // Get the id of the newly inserted image
                        $lastInsertedId = insertGlove($cat, $brand, $color, $name, $desc);
                        if (!$lastInsertedId) {
                            // Couldn't get the lastInsertedId
                            // Glove was not inserted
                            $errs[] = "Failed to insert glove";
                            $errCodes[] = 422;
                        } else {
                            // Got the ID of the inserted Glove
                            $isSizeInserted = insertGloveSizes($sizes, $lastInsertedId);
                            if (!$isSizeInserted) {
                                // Failed to insert sizes for glove
                                $errs[] = "Failed to insert glove sizes";
                                $errCodes[] = 422;
                            } else {
                                // Inserted sizes for glove
                                $isPriceInserted = insertGlovePrice($price, $lastInsertedId);
                                if (!$isPriceInserted) {
                                    // Failed to insert price for glove
                                    $errs[] = "Failed to insert glove price";
                                    $errCodes[] = 422;
                                } else {
                                    // Price was inserted 
                                    $isImageInsert = insertImages($n_name, $t_name, $lastInsertedId);
                                    if (!$isImageInsert) {
                                        // Failed to insert image
                                        $errs[] = "Failed to insert image";
                                        $errCodes[] = 422;
                                    } else {
                                        // Image was inserted
                                        // FINNISHED INSETING PRODUCT
                                        $success = "Glove updated succesfully";
                                    }
                                }
                            }
                        }
                    }
                }
            }

            // Rollback or Commit
            if (!empty($errs)) {
                // Rollback
                $conn->rollback();
                echo json_encode($errs, JSON_FORCE_OBJECT);
                http_response_code($errCodes[0]);
            } else {
                // Commit
                $conn->commit();
                if (isset($t_img)) {
                    // Send thumbnail image name, if exists
                    echo json_encode(['msg' => $success, "img" => $t_name, 'id' => $lastInsertedId], JSON_FORCE_OBJECT);
                } else {
                    // Image was not uploaded but is success
                    echo json_encode(['msg' => $success, 'id' => $lastInsertedId], JSON_FORCE_OBJECT);
                }
                http_response_code(201);
            }
        }
        // Catch
        catch (PDOException $ex) {
            // Try block failed
            $errs[] = $ex->getMessage();
            echo json_encode($errs, JSON_FORCE_OBJECT);
            http_response_code(422);
        }
    } else {
        // Form validation failed (checkboxes, radio buttons, input fileds, textarea)
        echo json_encode($errs, JSON_FORCE_OBJECT);
        http_response_code(422);
    }
} else {
    // Unathorized access
    $errs[] = "Unauthorized access";
    echo json_encode($errs, JSON_FORCE_OBJECT);
    http_response_code(403);
}
